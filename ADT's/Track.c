...
/*serán x > 0, x < 0 y 0 respectivamente. */
...
status_t ADT_Track_print_as_XML(ADT_Track_t *track, size_t init_tab, FILE *fo )
{	
	size_t c;
	...
	fprintf(fo, "%s%s"      , aux_tabs, TAG_START_TRACK);
	fprintf(fo, "%s%c%s%s%s", aux_tabs, TRACK_FIRST_BRANCH_TAB, TAG_START_TITLE, track->title, TAG_END_TITLE);
	fprintf(fo, "%s%c%s%s%s", aux_tabs, TRACK_FIRST_BRANCH_TAB, TAG_START_ARTIST, track->artist, TAG_END_ARTIST );
	fprintf(fo, "%s%c%s%s%s", aux_tabs, TRACK_FIRST_BRANCH_TAB, TAG_START_GENRE, genres[(int)track->genre], TAG_END_GENRE);
	fprintf(fo, "%s%c%s%s%s", aux_tabs, TRACK_FIRST_BRANCH_TAB, TAG_START_YEAR, track->year, TAG_END_YEAR);
