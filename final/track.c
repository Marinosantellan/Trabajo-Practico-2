...
status_t ADT_Track_load_from_mp3(ADT_Track_t *track, FILE* fi)
{
	size_t len;
	char *temp;
	char aux_year[INDEX_SPAN_YEAR+1];
	char aux_genre[INDEX_SPAN_GENRE+1];
    ...
    memcpy(aux_year,header+INDEX_START_YEAR,INDEX_SPAN_YEAR);
    aux_year[INDEX_SPAN_YEAR+1] = NUL;

    memcpy(track->comment,header+INDEX_START_COMMENT,INDEX_SPAN_COMMENT);
    track->comment[INDEX_SPAN_COMMENT+1] = NUL;

    memcpy(aux_genre,header+INDEX_START_YEAR,INDEX_SPAN_YEAR);
    aux_genre[INDEX_SPAN_GENRE+1] = NUL;

    track->year = strtoul(aux_year, &temp, 10);
    track->genre = strtoul(aux_genre, &temp, 10);
...
int ADT_Track_compare_by_year(const ADT_Track_t *track_1, const ADT_Track_t *track_2)
{
	...
	return track_1->year - track_2->year;
}

int ADT_Track_compare_by_genre(const ADT_Track_t *track_1, const ADT_Track_t *track_2)
{
	if(track_1 == NULL || track_2 == NULL)
	{
		return ERROR_NULL_POINTER;
	}
	return track_1->genre - track_2->genre;
}

status_t ADT_Track_print_as_csv(const ADT_Track_t *track, const char *delimiter, FILE * fo)
{
	...
	fprintf(fo, "%s%c%s%c%s%c%lu",track->title,*delimiter,track->artist,*delimiter,genres[track->genre],*delimiter,track->year);

	return OK;
}

status_t ADT_Track_print_as_xml(const ADT_Track_t *track, const size_t *init_tab, FILE *fo)
{	
	size_t c;
	...
	fprintf(fo, "%s%c%s%s%s\n", aux_tabs, TRACK_FIRST_BRANCH_TAB, TAG_START_GENRE, genres[track->genre], TAG_END_GENRE);
	fprintf(fo, "%s%c%s%lu%s\n", aux_tabs, TRACK_FIRST_BRANCH_TAB, TAG_START_YEAR, track->year, TAG_END_YEAR);
	fprintf(fo, "%s%s"	  	  , aux_tabs, TAG_END_TRACK);

	return OK;	
}

int strcmp_optimized(const char *str_1, const char *str_2)
{
	char char_1, char_2;
	size_t i;
